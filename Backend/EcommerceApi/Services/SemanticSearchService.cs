...
namespace EcommerceApi.Services
{
    ...
    {
        // Custom TF-IDF based vectorizer that runs 100% locally.
        // It provides a real mathematical Vector Embedding space (high-dimensional float arrays) 
        // and calculates Cosine Similarity over them, giving an authentic RAG/semantic search experience.
        ...
        public void ComputeAndStoreProductVectors(List<Product> products)
        {
            ...
            // Tokenize all descriptions to build vocabulary
            ...
            // Compute vectors for each product
        ...
        public List<(Product Product, double Score)> Search(List<Product> products, string query, int topK = 4)
        {
            ...
            foreach (var product in products)
            {
                ...
                // Add a substring and prefix boost for queries to handle partial words (e.g., "neb" -> "Nebula")
                double boost = 0.0;
                ...
                else
                {
                    ...
                    {
                        // Ignore short query words like "do", "you", "the"
            ...
            // Return top K sorted by similarity score descending
        ...
        private List<string> Tokenize(string text)
        {
            ...
            return cleanText.Split(new[] { ' ', '\r', '\n', '\t' }, StringSplitOptions.RemoveEmptyEntries)
                            .Where(w => w.Length > 2) // Filter out small words like 'a', 'in', 'to'
                            .ToList();
        }
